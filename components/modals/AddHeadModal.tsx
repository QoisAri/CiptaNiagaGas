...
import { addHead, type FormState } from '../../app/head/actions'
...
export default function AddHeadModal({ isOpen, onClose }: AddHeadModalProps) {
  ...
  const [state, formAction] = useFormState(addHead, initialState)
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl z-50 w-full max-w-md">
        ...
        <form ref={formRef} action={formAction} className="space-y-4">
          <div>
            ...
            <input type="text" id="head_code" name="head_code" required className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-black"/>
          </div>
          <div>
            <label htmlFor="type" className="block text-sm font-medium">Tipe</label>
            <input type="text" id="type" name="type" placeholder="Contoh: Arm Roll" className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-black"/>
          </div>
          <div>
            <label htmlFor="feet" className="block text-sm font-medium">Ukuran Feet</label>
            <select id="feet" name="feet" required className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-black">
