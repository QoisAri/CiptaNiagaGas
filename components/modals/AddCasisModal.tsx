...
import { addCasis } from '@/app/casis/actions'
...
export default function AddCasisModal({ isOpen, onClose }: AddCasisModalProps) {
  ...
  const [state, formAction] = useActionState(addCasis, initialState)
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl z-50 w-full max-w-md">
        ...
        <form ref={formRef} action={formAction} className="space-y-4">
          <div>
            ...
            <input type="text" id="chassis_code" name="chassis_code" required className="mt-1 block w-full border border-gray-300 rounded-md p-2"/>
          </div>
          <div>
            <label htmlFor="type" className="block text-sm font-medium">Tipe</label>
            <input type="text" id="type" name="type" placeholder="Contoh: 2-Axle 20ft" required className="mt-1 block w-full border border-gray-300 rounded-md p-2"/>
          </div>
