...
export default function PendingRepairClient({ initialData }: { initialData: PendingRepairItem[] }) {
  ...
  const filteredData = useMemo(() => {
    return initialData.filter(item => {
      ...
      return item.tipeAset.toLowerCase() === filterAsset;
    });
  }, [initialData, filterAsset]);

  // ## FUNGSI UNTUK MENGUNDUH EXCEL (SUDAH BENAR) ##
  const handleDownload = async () => {
    if (isDownloading) return;
    setIsDownloading(true);
    try {
      const { file, fileName } = await generatePendingRepairReport(filteredData);
  ...
  // ## FUNGSI UNTUK MENAMPILKAN FOTO (DIPERBAIKI) ##
  const handleShowImage = (url: string) => {
    // Fungsi ini hanya bertugas membersihkan URL dan menampilkan modal
    ...
    <>
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-4">
          ...
          {/* Tombol ini sekarang memanggil fungsi handleDownload yang benar */}
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 border-b text-gray-900 text-center">
                      {item.problemPhotoUrl ? (
                        // Tombol ini sekarang memanggil fungsi handleShowImage yang benar
      ...
      {/* Kode modal gambar tidak berubah dan sudah benar */}
      {modalImageUrl && (
