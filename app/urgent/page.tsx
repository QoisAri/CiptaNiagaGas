...
import UrgentFixClient from './UrgentFixClient'; // FIX: Impor komponen klien baru

// FIX 1: Tambahkan problemPhotoUrl ke tipe data
export type ProblemReport = {
  ...
  status: string;
  deadline: string;
  problemPhotoUrl: string | null; // <-- Ditambahkan
};
...
async function getProblemReports(): Promise<ProblemReport[]> {
  ...
  const [
    { data: profiles }, { data: items }, { data: heads }, { data: chassis }, { data: storages }
  ] = await Promise.all([
  ...
  const finalData = reports.map(report => {
    ...
    return {
      ...
      // FIX 2: Tambahkan URL foto ke objek hasil
...
export default async function UrgentFixPage() {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      ...
      {/* FIX 3: Render komponen klien dan teruskan data laporan */}
      <UrgentFixClient reports={reports} />
    </div>
  );
}
