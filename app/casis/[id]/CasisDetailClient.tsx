...
// FIX: Pastikan path impor ini benar sesuai struktur folder Anda
import { upsertInspectionResult, generateCasisWordDoc } from '../actions'; // Pastikan path ini benar
...
type InspectionHeaderType = { id: string; tanggal: string; catatan: string | null; profiles: { name: string; } | null; chassis: { chassis_code: string; } | null; };
type Props = { inspectionHeader: InspectionHeaderType; groups: Group; deleteAction: (formData: FormData) => void; };

// Komponen SubmitButton dan ItemRow tidak perlu diubah
...
function ItemRow({ row, inspectionId, pathname, onShowImage }: { row: Row, inspectionId: string, pathname: string, onShowImage: (url: string) => void }) {
  ...
  useEffect(() => { if (formState.success) setIsEditing(false); }, [formState]);
  const cleanedUrl = row.problem_photo_url?.replace(/([^:]\/)\/+/g, "$1") || '';
  return (
    <tr>
      <td className="border px-4 py-2 text-gray-800">{row.name}</td>
      <td className="border px-4 py-2 text-gray-800">{row.standard || '-'}</td>
      {isEditing ? (
        <>
        ...
        <>
          <td className="border px-4 py-2 text-gray-800">{row.kondisi}</td>
          <td className="border px-4 py-2 text-gray-800">{row.keterangan}</td>
        </>
      )}
      <td className="border px-4 py-2 text-center">
        {row.problem_photo_url ? (<button onClick={() => onShowImage(cleanedUrl)} className="text-blue-600 hover:underline"><FaImage className="inline-block h-5 w-5" /></button>) : ('-')}
      </td>
      <td className="border px-4 py-2 text-center space-x-2 no-print">
        {isEditing ? (
          <form id={formId} action={formAction}>
            <input type="hidden" name="resultId" value={row.resultId || undefined} /><input type="hidden" name="itemId" value={row.id} /><input type="hidden" name="inspectionId" value={inspectionId} /><input type="hidden" name="pathname" value={pathname} />
            <SubmitButton onCancel={() => setIsEditing(false)} />
          </form>
        ) : (<button type="button" onClick={() => setIsEditing(true)} className="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-1 px-3 rounded-md text-sm">Edit</button>)}
      </td>
    </tr>
  );
}
...
export function CasisDetailClient({ inspectionHeader, groups, deleteAction }: Props) {
  ...
  // Fungsi baru untuk memanggil server action
  const handleDownloadWord = async () => {
    if (isDownloading) return;
    setIsDownloading(true);

    try {
      // Panggil server action dengan ID inspeksi
      const base64String = await generateCasisWordDoc(inspectionHeader.id);
      
      // Konversi Base64 string kembali menjadi file
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="print-content-wrapper">
          {Object.entries(groups).map(([pageTitle, subGroups]) => (
            <div key={pageTitle} className="mb-8">
              <h3 className="text-xl font-bold mb-4 text-gray-800 border-b pb-2">{pageTitle}</h3>
              {subGroups.map((subGroup) => (
                <div key={subGroup.parentName} className="mb-6">
                  <table className="w-full table-auto border-collapse">
                    <thead >
                      <tr>
                        <th colSpan={6} className="bg-gray-200 text-left p-2 font-bold text-gray-700 border">
                          {subGroup.parentName}
                        </th>
                      </tr>
                      <tr className="bg-gray-100">
                        <th className="border px-4 py-2 text-left font-bold text-gray-600 w-1/3">Item</th>
                        <th className="border px-4 py-2 text-left font-bold text-gray-600">Standard</th>
                        <th className="border px-4 py-2 text-left font-bold text-gray-600">Kondisi</th>
                        <th className="border px-4 py-2 text-left font-bold text-gray-600">Keterangan</th>
                        <th className="border px-4 py-2 text-center font-bold text-gray-600">Foto</th>
                        <th className="border px-4 py-2 text-center font-bold text-gray-600 w-48 no-print">Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
      ...
      {modalImageUrl && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 no-print" onClick={() => setModalImageUrl(null)}>
          <div className="relative p-4">
            <button onClick={() => setModalImageUrl(null)} className="absolute -top-10 -right-4 text-white text-3xl font-bold">&times;</button>
            <Image src={modalImageUrl} alt="Problem photo" width={800} height={600} className="max-w-screen-lg max-h-screen-lg object-contain"/>
          </div>
        </div>
      )}
    </>
