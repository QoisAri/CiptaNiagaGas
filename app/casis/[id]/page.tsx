...
// Tipe data lain yang dibutuhkan oleh komponen (biarkan seperti ini)
...
type InspectionItem = { id: string; name: string; standard: string | null; parent_id: string | null; page_title: string | null; };
...
type InspectionHeaderWithRelations = { id: string; tanggal: string; catatan: string | null; created_at: string; inspector_id: string; head_id: string | null; chassis_id: number | null; storage_id: number | null; chassis: { chassis_code: string; feet: number; } | null; profiles: { name:string; } | null; };
...
export default async function CasisDetailPage({
  ...
  const { data, error: headerError } = await supabase
    .from('inspections')
    .select( `*, chassis!inspections_chassis_id_fkey(chassis_code, feet), profiles!fk_inspector(name)`)
    .eq('id', inspectionId)
    .single();
  ...
  const { data: allMasterItems } = await supabase.from('inspection_items').select('*').eq('category', 'Chassis').ilike('name', searchPattern);
  const { data: inspectionResults } = await supabase.from('inspection_results').select('id, item_id, kondisi, keterangan, problem_photo_url').eq('inspection_id', inspectionId);

  const resultsMap = new Map((inspectionResults || []).map((result) => [
      result.item_id, { id: result.id, kondisi: result.kondisi, keterangan: result.keterangan, problem_photo_url: result.problem_photo_url, },
  ]));

  const itemsWithResults: ItemWithResult[] = (allMasterItems || []).map((item: InspectionItem) => {
    const result = resultsMap.get(item.id);
    return { ...item, resultId: result?.id || null, kondisi: result?.kondisi || 'Belum Diperiksa', keterangan: result?.keterangan || '-', problem_photo_url: result?.problem_photo_url || null, };
  });

  const groups: Group = {};
  const parentNameMap = new Map<string, string>();
  const parentIds = [...new Set(itemsWithResults.map((item) => item.parent_id).filter(Boolean))];
  ...
  for (const item of itemsWithResults) {
    ...
    const rowData: Row = { id: item.id, name: item.name, standard: item.standard, resultId: item.resultId, kondisi: item.kondisi, keterangan: item.keterangan, problem_photo_url: item.problem_photo_url, };
    if (item.parent_id) {
