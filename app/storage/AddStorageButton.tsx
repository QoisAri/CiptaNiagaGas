      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
            <h3 className="text-lg font-bold mb-4">Tambah Data Storage Baru</h3>
            <form ref={formRef} action={formAction}>
              <div className="space-y-4">
                <div>
                  <label htmlFor="storage_code" className="block text-sm font-medium text-gray-700">Storage Code (Contoh: MK-03)</label>
                  <input
                    type="text" id="storage_code" name="storage_code" required
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-black p-2"
                  />
                </div>
                <div>
                  <label htmlFor="type" className="block text-sm font-medium text-gray-700">Tipe (Contoh: 10 Feet)</label>
                  <input
                    type="text" id="type" name="type" required
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-black p-2"
                  />
                </div>
              </div>
